...
fun ToDestinationUserInput(onToDestinationChanged: (String) -> Unit) {
    val editTableUserInputState = rememberEditableUserInputState(hint = "Choose Destination")
    CraneEditableUserInput(
        state = editTableUserInputState,
        caption = "To",
    ...
    LaunchedEffect(
        key1 = editTableUserInputState,
        block = {
            snapshotFlow { editTableUserInputState.text }
                .filter { !editTableUserInputState.isHint }
                .collect {
                    currentOnDestinationChanged(editTableUserInputState.text)
                }
        }
    )
}
