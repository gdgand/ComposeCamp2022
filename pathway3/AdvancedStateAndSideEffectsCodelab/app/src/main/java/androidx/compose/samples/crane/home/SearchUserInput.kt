...
import androidx.compose.samples.crane.base.rememberEditableUser
...
fun ToDestinationUserInput(onToDestinationChanged: (String) -> Unit) {
    val editableUserInputState = rememberEditableUser(hint = "Choose Destination")
    ...
    val currentOnDestinationChagned by rememberUpdatedState(newValue = onToDestinationChanged)
    LaunchedEffect(editableUserInputState) {
        snapshotFlow { editableUserInputState.text }
            .filter { !editableUserInputState.isHint }
            .collect {
                currentOnDestinationChagned(editableUserInputState.text)
            }
    }
}
