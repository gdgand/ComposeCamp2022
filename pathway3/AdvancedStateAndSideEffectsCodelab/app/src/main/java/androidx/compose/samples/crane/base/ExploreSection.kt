    ...
    Surface(modifier = modifier.fillMaxSize(), color = Color.White, shape = BottomSheetShape) {
        Column(modifier = Modifier.padding(start = 24.dp, top = 20.dp, end = 24.dp)) {
            ...
            Box(Modifier.weight(1f)) {
                ...
                // 매 recomposition 마다 다시 계산한다. 이는 비효율적이다.
                // val showButton = listState.firstVisibleItemIndex > 0

                // 최초 state 기반으로 초기값을 설정한 이후 변경이 없다. 이는 의미없다.
                // val showButton = remember { listState.firstVisibleItemIndex > 0 }

                // state 변경에 따라서 값이 갱신되면서도 composition안에 계산 결과를 캐싱해 둬서 불필요한 recomposition을 막는 방법
