...
//    }
    val uiState by produceState(initialValue = DetailUiState(isLoading = true)) {
        val citiDetailsResult = viewModel.cityDetails
        value = if (citiDetailsResult is Result.Success<ExploreModel>) {
            DetailUiState(cityDetail = citiDetailsResult.data)
        } else {
    ...
    when {
        uiState.cityDetail != null -> {
            DetailsContent(
                exploreModel = uiState.cityDetail!!,
